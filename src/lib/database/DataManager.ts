...
interface DataManager {
  ...
  list<T extends DefaultDataReference>(collectionPath: string, queries?: any[]): Promise<T[]>;

  // Firestore verilerini belirli bir yol altında getirir
  get<T extends DefaultDataReference>(documentPath: string, queries?: any[]): Promise<T | undefined>;

  // Firestore verilerini belirli bir yol altında kendi oluşturduğu id değeri ile ekler
  add<T extends DefaultDataReference>(collectionPath: string, data: T): Promise<T>;

  // Firestore verilerini belirli bir yol altında ayarlar
  set<T extends DefaultDataReference>(documentPath: string, data: T): Promise<T>;

  // Firestore verilerini belirli bir yol altında günceller
  update<T extends DefaultDataReference>(documentPath: string, data: Partial<T>): Promise<T>;
  ...
  transaction<T extends DefaultDataReference>(transactionFunc: (transaction: any) => Promise<T>): Promise<T>;
}
