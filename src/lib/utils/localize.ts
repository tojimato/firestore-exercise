...
async function localize(language: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    const ref = collection(firestore, "languages", language);
...
async function example() {
  ...
  // Example usage
  console.log(Resource.TXT_ADD_BUTTON); // Output will be the localized text for the key "TXT_ADD_BUTTON" if available, otherwise "TXT_ADD_BUTTON" itself
}
